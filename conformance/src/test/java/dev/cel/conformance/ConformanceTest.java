...
public final class ConformanceTest extends Statement {
  ...
  private static CelChecker getChecker(SimpleTest test) throws Exception {
    ...
    for (com.google.api.expr.v1alpha1.Decl decl : test.getTypeEnvList()) {
      decls.add(Decl.parseFrom(decl.toByteArray(), ExtensionRegistry.getGeneratedRegistry()));
    }
  ...
  private static final CelRuntime RUNTIME =
      CelRuntimeFactory.standardCelRuntimeBuilder()
          ...
          .addLibraries(
              ...
              CelExtensions.strings(),
              CelOptionalLibrary.INSTANCE)
          .setEvaluateLinkedMessageTypes(true)
          .build();
      ...
      case OBJECT_VALUE:
        {
          ...
          Descriptor descriptor =
              TypeRegistry.getGeneratedTypeRegistry().getDescriptorForTypeUrl(object.getTypeUrl());
          Message prototype =
              MessageFactories.getImmutableMessageFactory().getPrototype(descriptor);
          Message message =
              prototype
                  .getParserForType()
                  .parseFrom(object.getValue(), ExtensionRegistry.getGeneratedRegistry());
          // Unwrap?
          return message;
        }
      case MAP_VALUE:
  ...
  private static Value toValue(Object object, CelType type) throws Exception {
    ...
    if (object instanceof com.google.api.expr.Value) {
      object =
          Value.parseFrom(
              ((com.google.api.expr.Value) object).toByteArray(),
    ...
    switch (test.getResultMatcherCase()) {
      case VALUE:
        ...
        assertThat(result)
            ...
            .unpackingAnyUsing(
                TypeRegistry.getGeneratedTypeRegistry(), ExtensionRegistry.getGeneratedRegistry())
