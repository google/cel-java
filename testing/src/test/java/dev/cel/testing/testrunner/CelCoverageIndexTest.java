...
public final class CelCoverageIndexTest {
  ...
  public void getCoverageReport_fullCoverage() throws Exception {
    ...
    coverageIndex.setAst(ast);
    CelEvaluationListener listener = coverageIndex.getEvaluationListener();

    program.trace(ImmutableMap.of("x", 2L, "y", 2L), listener);

    CoverageReport report = coverageIndex.getCoverageReport();
    ...
    assertThat(report.coveredBooleanOutcomes).isEqualTo(3); // x>1 -> true, y>1 -> true, && -> true
    assertThat(report.unencounteredNodes).isEmpty();
    assertThat(report.unencounteredBranches)
        .containsExactly(
            "Expression ID 4 ('x > 1 && y > 1'): Never evaluated to 'false'",
            "\t\tExpression ID 2 ('x > 1'): Never evaluated to 'false'",
            "\t\tExpression ID 6 ('y > 1'): Never evaluated to 'false'");
  }
  ...
  public void getCoverageReport_partialCoverage_shortCircuit() throws Exception {
    ...
    CoverageReport report = coverageIndex.getCoverageReport();
    assertThat(report.celExpr).isEqualTo("x > 1 && y > 1");
    assertThat(report.nodes).isGreaterThan(0);
    ...
    assertThat(report.unencounteredBranches)
        .containsExactly(
            "Expression ID 4 ('x > 1 && y > 1'): Never evaluated to 'true'",
            "\t\tExpression ID 2 ('x > 1'): Never evaluated to 'true'");
  }
}
