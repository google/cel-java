...
public abstract class BaseInterpreterTest extends CelBaselineTestCase {
  ...
  protected static final Descriptor TEST_ALL_TYPE_DYNAMIC_DESCRIPTOR = getDeserializedTestAllTypeDescriptor();
  ...
  private static Descriptor getDeserializedTestAllTypeDescriptor() {
    String fdsContent;
    try {
      fdsContent = readResourceContent("testdata/proto3/test_all_types.fds");
      ...
      for (FileDescriptor f : fileDescriptors) {
        Descriptor descriptor = f.getMessageTypes().stream().filter(
                x -> x.getFullName().equals("dev.cel.testing.testdata.serialized.proto3.TestAllTypes"))
            .findAny().orElse(null);
        if (descriptor != null) {
          return descriptor;
        }
    ...
    throw new IllegalStateException("Could not find deserialized TestAllTypes descriptor.");
  }
  ...
  public void dynamicMessage_dynamicDescriptor() throws Exception {
    ...
    assertThat(runTest(Activation.EMPTY)).isInstanceOf(
        eval.celOptions().enableUnsignedLongs() ? UnsignedLong.class : Long.class);
    ...
    declareVariable("any_packed_test_msg",
        CelTypes.createMessage(TestAllTypes.getDescriptor().getFullName()));
    source = "TestAllTypes { single_any: any_packed_test_msg }.single_any";
    assertThat(runTest(Activation.of("any_packed_test_msg", anyTestMsg))).isInstanceOf(
        TestAllTypes.class);

    // Test JSON map behavior
    declareVariable("test_msg",
        CelTypes.createMessage(TestAllTypes.getDescriptor().getFullName()));
    declareVariable("dynamic_msg",
        CelTypes.createMessage(TEST_ALL_TYPE_DYNAMIC_DESCRIPTOR.getFullName()));
    DynamicMessage.Builder dynamicMessageBuilder = DynamicMessage.newBuilder(
        TEST_ALL_TYPE_DYNAMIC_DESCRIPTOR);
