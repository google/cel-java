...
public final class TestRunnerLibrary {
  ...
  private static Cel extendCel(Cel cel, CelOptions celOptions) throws Exception {
    ...
    if (fileDescriptorSetPath != null) {
      ...
      ImmutableList<Descriptor> compiledDescriptors = RegistryUtils.loadDescriptorsFromJvm();

      ImmutableList.Builder<FileDescriptor> compiledFileDescriptors = ImmutableList.builder();
      for (Descriptor descriptor : compiledDescriptors) {
        for (FileDescriptor fileDescriptor : fileDescriptors) {
          if (fileDescriptor.getFullName().equals(descriptor.getFile().getFullName())) {
            compiledFileDescriptors.add(fileDescriptor);
            break;
          }
        }
      }
      ...
      FileDescriptorSet fileDescriptorSet =
          FileDescriptorSet.newBuilder()
              .addAllFile(
                  compiledFileDescriptors.build().stream()
      ...
      extendedCel =
          cel.toCelBuilder()
              .addFileTypes(fileDescriptorSet)
              .setExtensionRegistry(RegistryUtils.getExtensionRegistry(fileDescriptors))
              .build();
    }
      ...
      case NO_INPUT:
        ...
        for (Map.Entry<String, Object> entry : celTestContext.variableBindings().entrySet()) {
          if (entry.getValue() instanceof Any) {
            newBindings.put(
                entry.getKey(),
                unpackAny((Any) entry.getValue(), System.getProperty("file_descriptor_set_path")));
          } else {
            newBindings.put(entry.getKey(), entry.getValue());
          }
        }
  ...
  private static Message unpackAny(Any any, String fileDescriptorSetPath) throws IOException {
    ...
    logger.info("Descriptor: " + descriptor.getFullName());
    ...
    logger.info("Default instance: " + defaultInstance.getDescriptorForType().getFullName());

    Message message =
        defaultInstance.getParserForType().parseFrom(any.getValue(), extensionRegistry);
    logger.info("Unpacked Any: " + message);
    return message;
  }
