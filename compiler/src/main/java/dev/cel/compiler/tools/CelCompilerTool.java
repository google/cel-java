...
/**
 * CelCompilerTool is a binary that takes a CEL expression in string, compiles it into a dev.cel.expr.CheckedExpr protobuf message, then writes the content to a .binary pb file.
 */
final class CelCompilerTool implements Callable<Integer> {

  @Option(names = {"--cel_expression"}, description = "CEL expression")
  ...
  @Option(names = {"--output"}, description = "Output path for the compiled binarypb")
  ...
  static CelAbstractSyntaxTree compile(String expression, String transitiveDescriptorSetPath)
  ...
  static void writeCheckedExpr(CelAbstractSyntaxTree ast, String filePath)
      throws IOException {
  ...
  public Integer call() {
    try {
      ...
      String errorMessage = String.format("Expression [%s] failed to compile. Reason: %s", celExpression, e.getMessage());
