...
def java_lite_proto_cel_library(
    ...
    Args:
       ...
       descriptor_class_prefix: Prefix name for the generated descriptor java class (ex: 'TestAllTypes' generates 'TestAllTypesCelLiteDescriptor.java').
       deps: Name of the proto_library target. Only a single proto_library is supported at this time.
       debug: If true, prints additional information during codegen for debugging purposes.
    """
    if not deps:
