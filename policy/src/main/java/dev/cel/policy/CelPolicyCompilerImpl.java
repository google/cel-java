...
final class CelPolicyCompilerImpl implements CelPolicyCompiler {
  ...
  private static final class CompilerContext {
    ...
    private CelSourceLocation computeAbsoluteLocation(long id, CelIssue issue) {
      int policySourceOffset = Optional.ofNullable(celPolicySource.getPositionsMap().get(id)).orElse(-1);
      CelSourceLocation policySourceLocation = celPolicySource.getOffsetLocation(policySourceOffset).orElse(null);
      ...
      return
          celPolicySource.getOffsetLocation(absoluteOffset + absoluteColumn).orElse(CelSourceLocation.NONE);
    }
