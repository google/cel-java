...
final class RuleComposer implements CelAstOptimizer {
  ...
  private CelMutableAst inlineCompiledVariables(CelMutableAst ast, List<CelCompiledVariable> compiledVariables) {
    ...
    for (CelCompiledVariable compiledVariable : Lists.reverse(compiledVariables)) {
      ...
      ImmutableList<CelNavigableMutableExpr> exprsToReplace = CelNavigableMutableAst.fromAst(mutatedAst).getRoot().allNodes().filter(
          node -> node.expr().getKind().equals(Kind.IDENT) && node.expr().ident().name().equals(variableName))
          .collect(toImmutableList());
