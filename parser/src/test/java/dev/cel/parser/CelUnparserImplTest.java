  ...
  public void unparse_macroWithReceiverStyleArg() throws Exception {
    CelParser parser =
            CelParserImpl.newBuilder()
                    ...
                    .setStandardMacros(CelStandardMacro.STANDARD_MACROS)
                    .build();
    CelAbstractSyntaxTree ast = parser.parse("[\"a\"].all(x, x.trim().lowerAscii().contains(\"b\"))").getAst();

    assertThat(unparser.unparse(ast)).isEqualTo("[\"a\"].all(x, x.trim().lowerAscii().contains(\"b\"))");
  }
}
